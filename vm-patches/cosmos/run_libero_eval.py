...
class PolicyEvalConfig:
    ...
    task_ids: Optional[str] = None                                       # Optional comma-separated task ids for smoke evals
    prompt_override: Optional[str] = None                                # Optional language instruction override for prompt sensitivity tests
...
def run_episode(
    ...
    if cfg.data_collection:
        ...
        actions_list = []
        query_t_list = []
        query_primary_images_list = []
        query_wrist_images_list = []
        query_proprio_list = []
        query_action_chunks_list = []
        query_future_primary_images_list = []
        query_future_wrist_images_list = []
        query_value_list = []
    ...
    try:
        ...
        while t < max_steps + NUM_STEPS_WAIT:
            ...
            if len(action_queue) == 0:
                ...
                if cfg.data_collection:
                    query_t_list.append(t - NUM_STEPS_WAIT)
                    query_primary_images_list.append(observation["primary_image"])
                    if cfg.use_wrist_image:
                        query_wrist_images_list.append(observation["wrist_image"])
                    query_proprio_list.append(observation["proprio"])
                    query_action_chunks_list.append(np.asarray(best_actions, dtype=np.float32))
                    query_value_list.append(float(best_value_predictions))
                    if cfg.use_third_person_image and best_future_predictions.get("future_image") is not None:
                        query_future_primary_images_list.append(best_future_predictions["future_image"])
                    if (
                        cfg.use_wrist_image
                        and best_future_predictions.get("future_wrist_image") is not None
                    ):
                        query_future_wrist_images_list.append(best_future_predictions["future_wrist_image"])
    ...
    if cfg.data_collection:
        ...
        if len(query_t_list) > 0:
            ...
            collected_data["query_primary_images"] = np.stack(query_primary_images_list, axis=0)
            if cfg.use_wrist_image and len(query_wrist_images_list) > 0:
                collected_data["query_wrist_images"] = np.stack(query_wrist_images_list, axis=0)
            collected_data["query_proprio"] = np.stack(query_proprio_list, axis=0)
            collected_data["query_action_chunks"] = np.stack(query_action_chunks_list, axis=0)
            collected_data["query_values"] = np.asarray(query_value_list, dtype=np.float32)
            if len(query_future_primary_images_list) > 0:
                collected_data["query_future_primary_images"] = np.stack(query_future_primary_images_list, axis=0)
            if len(query_future_wrist_images_list) > 0:
                collected_data["query_future_wrist_images"] = np.stack(query_future_wrist_images_list, axis=0)
...
def run_task(
    ...
    if cfg.prompt_override is not None:
        ...
        log_message(
            f"Using prompt override for task {task_id}: canonical='{canonical_task_description}' override='{task_description}'",
            log_file,
        )
...
def eval_libero(cfg: PolicyEvalConfig) -> float:
    ...
    if cfg.task_ids is not None:
        task_ids = [int(x.strip()) for x in cfg.task_ids.split(",") if x.strip()]
        invalid_task_ids = [task_id for task_id in task_ids if task_id < 0 or task_id >= num_tasks]
        if invalid_task_ids:
            raise ValueError(f"Invalid task_ids for {cfg.task_suite_name}: {invalid_task_ids}")
        log_message(f"Using explicit task_ids: {task_ids}", log_file)
