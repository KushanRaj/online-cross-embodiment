...
def main(eval_args: EvalArgs):
    ...
    if os.environ.get("POLARIS_EPISODE_LENGTH_S"):
        env_cfg.episode_length_s = float(os.environ["POLARIS_EPISODE_LENGTH_S"])
        print(f"Using episode_length_s={env_cfg.episode_length_s}")
    ...
    canonical_language_instruction = language_instruction
    if eval_args.instruction:
        language_instruction = eval_args.instruction
        print(
            "Using instruction override: "
            f"canonical={canonical_language_instruction!r} "
            f"override={language_instruction!r}"
        )
    ...
    while True:
        ...
        if term[0] or trunc[0] or bar.n >= horizon:
            ...
            episode_data = {
                ...
                "canonical_instruction": canonical_language_instruction,
