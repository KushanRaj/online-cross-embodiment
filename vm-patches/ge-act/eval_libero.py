...
class InferenceLibero:
    ...
    def infer(self, num_trails_per_task, image_shape=(256,256), task_ids=None, prompt_override=None):
        total_episodes, total_successes = 0, 0
        if task_ids is None:
            task_ids = range(self.task_suite.n_tasks)
        else:
            task_ids = [int(task_id.strip()) for task_id in task_ids.split(",") if task_id.strip()]
            invalid_task_ids = [
                task_id for task_id in task_ids
                if task_id < 0 or task_id >= self.task_suite.n_tasks
            ]
            if invalid_task_ids:
                raise ValueError(f"Invalid task_ids for {self.task_suite_name}: {invalid_task_ids}")
        for task_id in task_ids:
            ...
            if prompt_override is not None:
                task_description = prompt_override
                print(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_ids", type=str, default=None, help="comma-separated task ids for smoke evals")
    parser.add_argument("--prompt_override", type=str, default=None, help="optional language instruction override")
    ...
    libero_infer.infer(
        ...
        image_shape=libero_infer.args.data["train"]["sample_size"],
